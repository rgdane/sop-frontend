...
import { wordListItem } from "./items";
import DollarNodeView from "./DollarNode";

const extensionName = "dollarCommand";
let popup: any;

const DollarCommand = Extension.create({
  name: extensionName,
  priority: 200,
  ...
  addProseMirrorPlugins() {
    return [
      Suggestion({
        ...
        char: "$",
        allowSpaces: false,
        startOfLine: false, // Sudah false, tapi allow masih restrict
        pluginKey: new PluginKey(extensionName),
        allow: ({ state, range }) => {
          const $from = state.doc.resolve(range.from);
          const isParagraph = $from.parent.type.name === "paragraph";

          // Cek apakah sedang di dalam column
          const isInColumn = this.editor.isActive("column");

          // Cek apakah ada double space setelah $
          const afterContent = $from.parent.textContent?.slice(
            Math.max(0, $from.parent.textContent?.indexOf("$"))
          );
          const isValidAfterContent = !afterContent?.endsWith("  ");

          // Allow $ di mana saja selama:
          // 1. Di dalam paragraph
          // 2. Tidak ada double space setelahnya
          return isParagraph && isValidAfterContent;
        },
        command: ({
          editor,
          range,
        ...
        render: () => {
          ...
          return {
            onStart: (props: SuggestionProps) => {
              component = new ReactRenderer(DollarNodeView, {
                props,
                editor: props.editor,
              });
